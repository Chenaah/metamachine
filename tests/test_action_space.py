...
from metamachine.environments.components.action import ActionBounds, ActionProcessor
...
class TestActionProcessor:
    ...
    def test_action_scaling(self, basic_config):
        ...
        expected = np.array([1.0, -1.0, 0.0]) + np.array(
            basic_config.control.default_dof_pos
        )
    ...
    def test_action_clipping(self, basic_config):
        ...
        expected = np.array([0.5, -0.5, 0.25]) + np.array(
            basic_config.control.default_dof_pos
        )
        np.testing.assert_array_equal(processed, expected)
    ...
    def test_frozen_joints(self, basic_config):
        ...
        expected = np.array([0.0, 0.2, 0.0]) + np.array(
            basic_config.control.default_dof_pos
        )
        np.testing.assert_array_equal(processed, expected)
    ...
    def test_processing_pipeline(self, basic_config):
        ...
        processor = ActionProcessor(basic_config)
        processor.smoother.reset(
            init_hist=np.array([0.29616328, -0.62756255, 0.38721781])
        )
...
class TestActionSpaceIntegration:
    ...
    def test_action_space_creation(self, mock_kbhit, test_config):
        ...
        np.testing.assert_allclose(
            env.action_space.low, np.full(5, expected_low, dtype=np.float32)
        )
        np.testing.assert_allclose(
            env.action_space.high, np.full(5, expected_high, dtype=np.float32)
        )
    ...
    def test_action_bounds_enforcement(self, mock_kbhit, test_config):
        ...
        np.testing.assert_array_equal(
            env.action_processor.last_action_full, expected_clipped
        )
