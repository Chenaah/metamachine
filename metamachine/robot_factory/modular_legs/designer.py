...
from .connect import (
    ConnectAsym,
    generate_stick_docks,
    generate_stick_to_stick_child_connections,
)
...
class RobotDesigner:
    ...
    def __init__(
        ...
        self.robot_cfg["length"] = self.robot_cfg["l_"] - (
            self.robot_cfg["R"]
            - np.sqrt(self.robot_cfg["R"] ** 2 - self.robot_cfg["r"] ** 2)
        )
        ...
        for connection_type in connection_types:
            ...
            for dock in docks:
                ...
                T_A_B = calculate_transformation_matrix(
                    dock.pos_a, dock.rotate_a, dock.pos_b, dock.rotate_b
                )
                pos, quat = matrix_to_pos_quat(T_A_B)
                self.available_ports[parent_id][position_a][position_b][
                    connection_type
                ].append((pos, quat))
    ...
    def _add_a_ball(
        ...
        if parent_id is None:
            ...
            node_ids = self.builder.add_simple_ball(
                color=self.color, range=None, passive=passive
            )
        else:
            ...
            ports = self.available_ports[parent_id][position_id_a][position_id_b][
                connection_type
            ]
    ...
    def _add_sites(self, xml_body: etree.Element) -> None:
        ...
        for dock in docks:
            ...
            etree.SubElement(
                ...
                name=f"{xml_body.get('name')}_{dock.name}",
            )  # TODO: move to RobotBuilder
    ...
    def get_rotation_id_list(
        self, module: int, parent_id: int, position_id_a: int, position_id_b: int
    ) -> list[int]:
        """
        ...
        return list(
            range(
                len(
                    self.available_ports[parent_id][position_id_a][position_id_b][
                        connection_type
                    ]
                )
            )
    ...
    def reset(self, init_a_ball: bool = True) -> None:
        ...
        self.builder = RobotBuilder(
            mesh_dict=self.mesh_dict, robot_cfg=self.robot_cfg, sim_cfg=self.sim_cfg
        )
        self.connection_types = ["00", "02", "10", "12", "20", "22"]
        self.color = 3
        self.available_ports = defaultdict(
    ...
    def compile(
        ...
        try:
            ...
            if stable_state_test:
                stable_state_check(
                    m, d, self.robot_properties, self.robot_cfg["theta"], viewer=viewer
                )
...
def test_general_usage():
    ...
    for pipeline in pipelines:
        ...
        pos = pipeline[2]
        robot_designer.get_rotation_id_list(module, parent, pos)
        orientation = pipeline[3]

        # Execute step
        robot_designer.step(pipeline)
        print(
            f"Added module: type={module}, parent={parent}, pos={pos}, rot={orientation}"
        )
...
def test_debug():
    ...
    robot_designer = RobotDesigner(
        robot_cfg=ROBOT_CFG_AIR1S, mesh_dict=MESH_DICT_FINE, allow_overlapping=True
    )
    robot_designer.reset()
