...
class ConnectAsym:
    ...
    def __init__(
        self, conn_type: Union[str, ConnectionType], robot_cfg: dict, lite: bool = False
    ):
        ...
        if conn_type in [
            ConnectionType.UP_BALL_TO_UP_BALL,
            ConnectionType.BOTTOM_BALL_TO_UP_BALL,
            ConnectionType.UP_BALL_TO_STICK,
            ConnectionType.BOTTOM_BALL_TO_STICK,
        ]:
            # ball <- ball or ball <- stick
            # screw_thetas = [0, 2*pi/3, 4*pi/3, pi/3, pi, 5*pi/3] # screw the ball at each docking position
            self._generate_ball_connections(
                conn_type,
                R,
                r,
                delta_l,
        ...
        # elif conn_type == "20":
            ...
            # stick <- ball
            # screw_thetas = [0, 2*pi/3, 4*pi/3, pi/3, pi, 5*pi/3] # screw the ball at each docking position
            self._generate_stick_to_ball_connections(
                conn_type,
                R,
                r,
                delta_l,
        ...
        # elif conn_type == "22":
            ...
            # stick <- stick
            # screw_thetas = [0, 2*pi/3, 4*pi/3, pi/3, pi, 5*pi/3] # screw the ball at each docking position
            self._generate_stick_to_stick_connections(
                conn_type, R, r, delta_l, theta, length, stick_ball_l, screw_thetas
            )

    def _generate_ball_connections(
        self,
        conn_type,
        R,
        r,
        ...
        for i, dock_theta_a in enumerate(dock_pos):
            for screw_theta in screw_thetas:
                pos_a = [
                    (R - delta_l) * cos(theta) * sin(dock_theta_a),
                ...
                if conn_type in [
                    ...
                    for j, dock_theta_b in enumerate(dock_pos):
                        pos_b = [
                            (R - delta_l) * cos(theta) * sin(dock_theta_b),
                        ...
                        self.dock_list.append(
                            DockingPoint(pos_a, pos_b, rotate_a, rotate_b, i, j)
                        )
                    ...
                    pos_counter_b = 0
                    pos_b = [0, 0, (length - stick_ball_l) / 2]
                    rotate_b = rotation_matrix_sequence(
                        [
                            rotation_matrix(
                                [cos(dock_theta_a), sin(dock_theta_a), 0],
                                -pi / 2 - theta,
                            ),
                    ...
                    self.dock_list.append(
                        DockingPoint(pos_a, pos_b, rotate_a, rotate_b, i, pos_counter_b)
                    )
                    ...
                    for m in range(3):
                        for j in self.side_j_list[m]:
                            pos_b = [
                                r * cos(j * pi / 2 + pi / 12),
                                r * sin(j * pi / 2 + pi / 12),
                                self.side_pos_list[m],
                            ]
                            rotate_b = rotation_matrix_sequence(
                                [
                                    rotation_matrix(
                                        [0, 0, 1], pi / 2 + j * pi / 2 - dock_theta_a
                                    ),
                                    rotation_matrix(
                                        [-sin(j * pi / 2), cos(j * pi / 2), 0], -theta
                                    ),
                                    rotation_matrix([1, 0, 0], pi * ((m + 1) % 2)),
                                    rotation_matrix(
                                        [0, 0, 1], pi / 12 + pi * ((m + 1) % 2)
                                    ),
                                ]
                            )
                            self.dock_list.append(
                                DockingPoint(
                                    pos_a, pos_b, rotate_a, rotate_b, i, pos_counter_b
                                )
                            )
    ...
    def _generate_stick_to_ball_connections(
        self,
        conn_type,
        R,
        r,
        ...
        # tip of stick a
        pos_a = [0, 0, (length - stick_ball_l) / 2]
        for screw_theta in screw_thetas:
            rotate_a = rotation_matrix_sequence(
                [
            ...
            for i, dock_theta_b in enumerate(dock_pos):
                ...
                self.dock_list.append(
                    DockingPoint(pos_a, pos_b, rotate_a, rotate_b, pos_counter, i)
                )
        ...
        for m in range(3):
            for j_a in self.side_j_list[m]:
                pos_a = [
                    r * cos(j_a * pi / 2 + pi / 12),
                    r * sin(j_a * pi / 2 + pi / 12),
                    self.side_pos_list[m],
                ]
                rotate_a = rotation_matrix_sequence(
                    [
                ...
                for screw_theta in screw_thetas:
                    for i, dock_theta_b in enumerate(dock_pos):
                        ...
                        self.dock_list.append(
                            DockingPoint(
                                pos_a, pos_b, rotate_a, rotate_b, pos_counter, i
                            )
                        )
    ...
    def _generate_stick_to_stick_connections(
        self, conn_type, R, r, delta_l, theta, length, stick_ball_l, screw_thetas
    ):
        ...
        for screw_theta in screw_thetas:
            ...
            self.dock_list.append(
                DockingPoint(
                    pos_a, pos_b, rotate_a, rotate_b, pos_counter, pos_counter_b
                )
            )
            pos_counter_b += 1
            ...
            for m in range(3):
                for j_b in self.side_j_list[m]:
                    pos_b = [
                        r * cos(j_b * pi / 2 + pi / 12),
                        r * sin(j_b * pi / 2 + pi / 12),
                        self.side_pos_list[m],
                    ]
                    rotate_b = Rotation.from_euler(
                        "yz", [-pi / 2, j_b * pi / 2 + pi / 12]
                    ).as_matrix()
                    # rotate_b = np.eye(3)
                    self.dock_list.append(
                        DockingPoint(
            ...
            pos_b = [0, 0, -(length - stick_ball_l) / 2]
            rotate_b = rotation_matrix_sequence([rotation_matrix([0, 0, 1], -pi / 6)])
            self.dock_list.append(
                DockingPoint(
                    pos_a, pos_b, rotate_a, rotate_b, pos_counter, pos_counter_b
                )
            ...
            for j_a in self.side_j_list[m]:
                ...
                for screw_theta in screw_thetas:
                    ...
                    rotate_b = rotation_matrix_sequence(
                        [
                            rotation_matrix(
                                [-sin(j_a * pi / 2), cos(j_a * pi / 2), 0], -pi / 2
                            ),
                        ...
                        for n in range(3):
                            for j_b in self.side_j_list[n]:
                                pos_b = [
                                    r * cos(j_b * pi / 2 + pi / 12),
                                    r * sin(j_b * pi / 2 + pi / 12),
                                    self.side_pos_list[n],
                                ]
                                rotate_b = rotation_matrix_sequence(
                                    [
                                        rotation_matrix([0, 1, 0], pi * (n % 2)),
                                        rotation_matrix(
                                            [0, 0, 1], j_b * pi / 2 - j_a * pi / 2
                                        ),
                                        rotation_matrix(
                                            [cos(j_b * pi / 2), sin(j_b * pi / 2), 0],
                                            screw_theta,
                                        ),
                                        rotation_matrix(
                                            [0, 0, 1], pi / 12 + pi * (n % 2) * (m % 2)
                                        ),
                                    ]
                                )
                                self.dock_list.append(
                    ...
                    rotate_b = rotation_matrix_sequence(
                        [
                            ...
                            rotation_matrix(
                                [0, 0, 1], -j_a * pi / 2 + screw_theta + pi / 6
                            ),
                            rotation_matrix([1, 0, 0], pi),
                        ]
                    )
        ...
        for screw_theta in screw_thetas:
            ...
            pos_b = [0, 0, -(length - stick_ball_l) / 2]
            rotate_b = rotation_matrix_sequence([rotation_matrix([1, 0, 0], pi)])
            self.dock_list.append(
                DockingPoint(
                    pos_a, pos_b, rotate_a, rotate_b, pos_counter, pos_counter_b
                )
...
def generate_stick_docks(robot_cfg):
    ...
    screw_thetas = [
        ...
        2 * pi / 3,
        4 * pi / 3,
    ]  # screw the ball at each docking position
    r = robot_cfg["r"] - stick_ball_l
    ...
    for m in range(3):
        for i, j_a in enumerate(side_j_list[m]):
            ...
            rotate_a = Rotation.from_euler(
                "xz", [pi * ((m + 1) % 2), pi / 12 + pi * (m % 2)]
            ).as_quat()
...
def generate_stick_to_stick_child_connections(robot_cfg):
    ...
    for m in range(3):
        for j_a in side_j_list[m]:
            for screw_theta in screw_thetas:
                pos_b = [0, 0, (length - stick_ball_l) / 2]
                first_rot = Rotation.from_rotvec(
                    np.array([-sin(j_a * pi / 2), cos(j_a * pi / 2), 0]) * -pi / 2
                )
                rotate_b = (
                    Rotation.from_euler("z", -j_a * pi / 2 + screw_theta + pi / 6)
                    * first_rot
                ).as_matrix()
                dock_list.append(DockingPoint(None, pos_b, None, rotate_b, None, None))

                pos_b = [0, 0, -(length - stick_ball_l) / 2]
                rot0 = Rotation.from_rotvec(
                    np.array([-sin(j_a * pi / 2), cos(j_a * pi / 2), 0]) * -pi / 2
                )
                rot1 = Rotation.from_euler(
                    "zx", [-j_a * pi / 2 + screw_theta + pi / 6, pi]
                )
                ...
                if screw_theta != 0:
                    for n in range(3):
                        for j_b in side_j_list[n]:
                            ...
                            rot0 = Rotation.from_euler(
                                "yz", [pi * (n % 2), j_b * pi / 2 - j_a * pi / 2]
                            )
                            rot1 = Rotation.from_rotvec(
                                np.array([cos(j_b * pi / 2), sin(j_b * pi / 2), 0])
                                * screw_theta
                            )
                            ...
                            dock_list.append(
                                DockingPoint(None, pos_b, None, rotate_b, None, None)
                            )

    # tip of stick a
    for _screw_theta in screw_thetas:
        pos_b = [0, 0, (length - stick_ball_l) / 2]
        rotate_b = np.eye(3)
        dock_list.append(DockingPoint(None, pos_b, None, rotate_b, None, None))
