...
class Visualizer:
    ...
    def _log_info(self, state: Any) -> None:
        """Log current state information."""
        self.info_dict = {
            "robot": f"{getattr(self.cfg, 'robot', {}).get('mode', 'unknown')} - {state.num_act} modules",
    ...
    def _generate_table(self) -> Table:
        ...
        for key, value in self.info_dict.items():
            ...
            table.add_row(
                key, str(value), self.status_dict[key], end_section=(key in end_keys)
            )
