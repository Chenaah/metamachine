...
class RecordVideo(
    ...
    def _capture_frame(self):
        ...
        # Use the new method if available, otherwise fall back to default
        if hasattr(self.unwrapped_env, "_get_viewer_with_camera"):
            viewer = self.unwrapped_env._get_viewer_with_camera("rgb_array")
        else:
            viewer = self.unwrapped_env.mujoco_renderer._get_viewer("rgb_array")

        viewer.cam.lookat = self.unwrapped_env.render_lookat_filter(
            self.unwrapped_env.pos_world
        )
        ...
        if len(self.recorded_frames) == 0:
            ...
            try:
                wandb.log(
                    {
                        "Video": wandb.Video(
                            np.transpose(np_frames, (0, 3, 1, 2)),
