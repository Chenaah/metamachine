...
def quat_rotate_inverse_jax(q: Any, v: Any) -> Any:
...
def quat_rotate_inverse_jax_wxyz(q: Any, v: Any) -> Any:
...
def quat_rotate_inverse_batch(q: Any, v: Any) -> Any:
    ...
    if not HAS_TORCH:
        raise ImportError(
            "PyTorch is required for this function. Install with: pip install torch"
        )
...
def quat_rotate_inverse(
    q: Union[list[float], np.ndarray], v: Union[list[float], np.ndarray]
) -> np.ndarray:
...
def calculate_angular_velocity(
    initial_quaternion: Union[list[float], np.ndarray],
...
class AverageFilter:
    ...
    def __init__(self, window_size: int) -> None:
        ...
        self.buffer: list[Union[float, NDArray[np.floating[Any]]]] = []
...
def velocity_transform_yaw(
    V_world: Union[list[float], np.ndarray], quaternion: Union[list[float], np.ndarray]
) -> np.ndarray:
...
def world_to_body_velocity_yaw(
    world_vel: Union[list[float], np.ndarray], quat: Union[list[float], np.ndarray]
) -> np.ndarray:
    """Transform velocity from world frame to body frame using yaw angle only.
    ...
    rotation_mat = np.array([[cos_yaw, -sin_yaw, 0], [sin_yaw, cos_yaw, 0], [0, 0, 1]])
...
def world_velocity_to_forward_body_velocity(
    quaternion: Union[list[float], np.ndarray], v_world: Union[list[float], np.ndarray]
) -> float:
...
def quaternion_multiply(
    q1: Union[list[float], np.ndarray], q2: Union[list[float], np.ndarray]
) -> np.ndarray:
...
def quat_apply_batch(a: Any, b: Any) -> Any:
...
def construct_quaternion(
    axis: Union[list[float], np.ndarray], angle: float, order: str = "wxyz"
) -> np.ndarray:
...
def quaternion_to_euler2(
    quaternion: Union[list[float], NDArray[np.floating[Any]]],
...
def quat_rotate_batch(q: Any, v: Any) -> Any:
    """Apply quaternion rotation to vectors (PyTorch batch version).
...
def ang_vel_to_ang_forward(
    ang_vel, projected_gravity, theta, alpha, onedir=True, return_y_vec=False
):
    # this is for the new module model
    length = 1
    v0 = np.array([0, length * np.cos(theta), length * np.sin(theta)])
    v1 = np.array(
        [length * np.sin(alpha), length * np.cos(alpha), -length * np.sin(theta)]
    )
...
def calculate_transformation_matrix(
    point_A: Union[list[float], NDArray[np.floating[Any]]],
    orientation_A: NDArray[np.floating[Any]],
    point_B: Union[list[float], NDArray[np.floating[Any]]],
    orientation_B: NDArray[np.floating[Any]],
...
def transform_point(
    T: NDArray[np.floating[Any]], point: Union[list[float], NDArray[np.floating[Any]]]
) -> NDArray[np.floating[Any]]:
    """
...
def rotation_matrix(
    axis: Union[list[float], NDArray[np.floating[Any]]], angle: float
) -> NDArray[np.floating[Any]]:
...
def rotation_matrix_multiply2(
    R1: NDArray[np.floating[Any]], R2: NDArray[np.floating[Any]]
) -> NDArray[np.floating[Any]]:
    return np.dot(R2, R1)


def rotation_matrix_sequence(
    r_list: list[NDArray[np.floating[Any]]],
) -> NDArray[np.floating[Any]]:
    R = np.eye(3)
    for r in r_list:
...
def quaternion_from_vectors(
    v1: Union[list[float], np.ndarray], v2: Union[list[float], np.ndarray]
) -> np.ndarray:
